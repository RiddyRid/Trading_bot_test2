...
def webhook():
    # Ручное чтение и парсинг JSON
    raw = request.data.decode('utf-8')
    try:
        data = json.loads(raw)
    except Exception as e:
        return {'error': f'Invalid JSON payload: {e}'}, 400

    sig = data.get('signal', '').lower()
    try:
        price = float(data.get('price', 0))
    except Exception:
        return {'error': 'Price is not a number'}, 400
